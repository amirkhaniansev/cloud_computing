...
namespace InformationCenterUI.HttpClients
{
    public class FilmClient
    {
        ...
        public async Task<int> AddTrailer(int id, byte[] content)
        {
            ...
            await blob.UploadFromByteArrayAsync(content, 0, content.Length);  // to API
            ...
            return await PostTrailer(trailer);
        }
        private async Task<int> PostTrailer(Trailer trailer)
        {
            HttpRequestMessage request = new HttpRequestMessage
            {
                Method = HttpMethod.Post,
                RequestUri = new Uri("Films/PostTrailer", UriKind.Relative),

                Content = new StringContent(JsonSerializer.Serialize(trailer), Encoding.UTF8, "application/json")
            };
            var result = await client.SendAsync(request);
            if (result.IsSuccessStatusCode)
            {
        ...
        public async Task<Trailer> GetTrailer(int id)
        {
            ...
            {
                Method = HttpMethod.Get,
                RequestUri = new Uri("Films/GetTrailer/" + id, UriKind.Relative)
            };
            ...
            {
                string cont = await result.Content.ReadAsStringAsync();
                return JsonSerializer.Deserialize<Trailer>(cont, new JsonSerializerOptions() { PropertyNameCaseInsensitive = true });
            }
            throw new Exception(await result.Content.ReadAsStringAsync());
        }
    }
